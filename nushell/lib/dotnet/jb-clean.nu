...
def clean [] {
  let modified = run-external  "git" "diff" "--name-only" "--relative" | lines;
  let staged = run-external  "git" "diff" "--name-only" "--relative" "--cached" | lines;
...
def "clean pr" [branch: string] {
  ...
  let changed = run-external "git" "diff" "--relative" "--name-only" $"HEAD..($branch)" | lines;
